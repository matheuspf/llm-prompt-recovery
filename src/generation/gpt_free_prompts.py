...
def build_prompt(content):
    prompt = f"""Generate a parseable json list of possible rewrite prompts a person may ask a LLM to do given an input text. The generated json will be in the format:
...
def get_prompts(num_iters=10, num_fewshot=5, max_runs=10, api_params={}):
    ...
    for iter_idx in tqdm(range(num_iters)):
        all_results += get_results(
            num_fewshot=num_fewshot, max_runs=max_runs, api_params=api_params
        )
