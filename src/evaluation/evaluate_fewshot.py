...
class FewshotPrompt:
    ...
    def __call__(self, dct, embd=None):
        ...
        prompt = f"""[INST] Given a list of Original Text, Rewritten Text, and the most likely Prompt given by a user to generate the Rewritten Text, generate the most likely Prompt for the last case.
...
'''
    ...
    def build_fewshot_prompt(self, dct, embd=None):
        ...
        fewshot_prompts = [
            self.build_singleshot_prompt(dct_fs) for idx, dct_fs in fewshot_examples.iterrows()
        ]
        # pred_prompt = self.build_singleshot_prompt(dct, add_subject=False)
