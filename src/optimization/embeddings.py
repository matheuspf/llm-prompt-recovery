...
def get_initial_embds(t5):
    ...
    embds = t5.encode(
        prompt, normalize_embeddings=True, show_progress_bar=False, convert_to_tensor=True
    )
...
def get_data_embds_cluster(t5, cluster_idx):
    df = pd.read_csv(
        "/home/edu/code/llm_style/data/cluster_models/2.6k_selected_prompts_with_clusters_spec.csv"
    )
    df = df[df[f"cluster_{NB_CLUSTERS}"] == cluster_idx].reset_index(drop=True)
...
def get_single_loss(t5, text, data_embds):
    embds = t5.encode(
        text, normalize_embeddings=True, show_progress_bar=False, convert_to_tensor=True
    )
...
def opt_embds(cluster_idx=-1, max_iter=100):
    ...
    optimizer = torch.optim.LBFGS(
        [embds], lr=LR, max_iter=max_iter, history_size=10, line_search_fn="strong_wolfe"
    )
...
def test_cluster_embds():
    ...
    for cluster_idx in clusters:
        ...
        nb_samples.append(len(data_embds))

    print(f"Mean loss: {np.mean(mean_loss)}")
    print(f"Mean weighted loss: {np.average(mean_loss, weights=nb_samples)}")
    print(mean_loss)
...
def run():
    ...
    inversion_model_path = (
        "/home/mpf/code/kaggle/vec2text/vec2text/saves/t5-prompts/checkpoint-10000/"
    )
    corrector_model_path = (
        "/home/mpf/code/kaggle/vec2text/vec2text/saves/t5-corrector-1/checkpoint-28000/"
    )

    inversion_model = vec2text.models.InversionModel.from_pretrained(inversion_model_path).to(
        device
    )
    ...
    output = vec2text.invert_embeddings(
        embeddings=embds[None].to(device), corrector=corrector, num_steps=100
    )[0]
